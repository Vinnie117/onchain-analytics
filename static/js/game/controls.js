function getPointerPos(e) {
  ...
  canvas.addEventListener("mousemove", (e) => {
    if (stone.isDragging) {
      ...
      if (dx > 0) mouseX = slingOrigin.x; // Don't allow dragging forward
  ...
  window.addEventListener("mouseup", () => {
    if (stone.isDragging) {
      ...
      stone.isFlying = true;
      stone.vx = (slingOrigin.x - stone.x) * 0.2; // reduced speed
      stone.vy = (slingOrigin.y - stone.y) * 0.2; // reduced speed
      slingSnapping = true;
      slingSnapBackProgress = 0;
    }
  });
